...
onready var transfer_all = $"%transfer_all"
...
func initialize():
	...
	transfer_all.connect("toggled", self, "_on_transfer_all_toggled")
	_on_transfer_all_toggled(transfer_all.pressed)
func _on_transfer_all_toggled(val):
	time_container.visible = !val

func _submit():
	var accum = play_timer.msec
	var all = transfer_all.pressed
	
	var time_to_transfer = accum if all else MathUtils.clampi(time.msec, 0, accum)
	
	play_timer.set_msec(accum-time_to_transfer)
	task_panel.register_work(title.text, time_to_transfer)
	emit_signal("submit")

func _on_msec_changed(_val):
